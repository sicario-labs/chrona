...
export default function CustomSearchDialog(props: SharedProps) {
  ...
  const pageTreeAction = useMemo<SearchItemType | undefined>(() => {
    ...
    for (const [k, page] of searchMap) {
      ...
      return {
        ...
        onSelect: () => navigate({ to: page.url }),
      };
    }
  }, [navigate, search, searchMap]);
  ...
  const searchItems = useMemo<SearchItemType[] | null>(() => {
    ...
    const items: SearchItemType[] = [
      ...
      ...data.map((result) => {
        ...
        return {
          id: result.id,
          type: 'action',
          node: <SymbolEvidence evidence={result._chrona} onOpen={() => navigate({ to: result.url })} />,
    ...
    return items;
  }, [query.data, pageTreeAction, navigate]);
