...
export const compileOptions: Partial<CompileOptions> = {
  onUnknownFile(absolutePath) {
    const filePath = toPosix(path.relative(baseDir, absolutePath));
  ...
  onParseReference(ref) {
    ...
    if (ref.type === 'file') {
      let file = toPosix(path.relative(baseDir, ref.file));

      if (file === 'lib/cn.ts') {
        return {
          type: 'file',
          file: path.join(baseUi.registry.dir, 'utils/cn.ts'),
        };
      }

      file = toPosix(path.relative(baseUi.registry.dir, ref.file));
      if (file.startsWith('contexts/') || file.startsWith('utils/use-')) {
        return {
          dep: '@chrona/base-ui',
          type: 'dependency',
          specifier: `@chrona/base-ui/${removeExtname(file)}`,
        };
      }
    }

    // map dep imports to actual components
    if (ref.type === 'dependency' && ref.dep === 'chrona-ui') {
      const match = /chrona-ui\/components\/ui\/(.*)/.exec(ref.specifier);

      if (match) {
        return {
          type: 'file',
          file: path.join(baseUi.registry.dir, `components/ui/${match[1]}.tsx`),
        };
      }
    }
