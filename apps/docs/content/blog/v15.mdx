...
Chrona v15 has no other significant changes other than migrating the config to Tailwind CSS v4.
...
Add `@import` to the presets exported by Chrona UI, and include the `chrona-ui` package in the source.
...
@source '../node_modules/chrona-ui/dist/**/*.js';
```
...
Chrona no longer use `--fd-<color>` like `--fd-primary: 0 0% 0%` for colors, it directly defines and uses colors in `@theme`.
...
v15 improves the playground UI (inspired by the minimalism of Scalar), and brought Scalar API Client support to Chrona OpenAPI.
...
import { APIPlayground } from 'chrona-openapi/scalar';
...
<Callout>
  Be careful that you must configure Tailwind CSS first, using the pre-built stylesheet of Chrona
...
In the future, we also want to make further improvements to Chrona MDX:
