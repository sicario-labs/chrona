...
We are pleased to announce the release of Chrona 16.2, introducing a better layout system for Chrona UI.

## The New Layout System

Handling layouts in documentation frameworks can present significant challenges.

As a solution, Chrona UI implemented a [Grid System](/docs/ui/layouts/docs#the-layout-system) that is composable, flexible, cohesive and predictable.
...
For user-land customizations, we recommend the Chrona CLI customize command, or hooking the relevant grid areas like:
...
Previously, Chrona UI relied on `position: fixed` and mathematical computations (inspired by Vitepress). The older method often lacked composability and difficult for beginners to manage.
...
While Chrona 16.2 preserves compatibility in several areas, notably:
...
If you have already installed layout components via Chrona CLI, replace `preset.css` with `preset-legacy.css` to keep legacy styles & avoid unexpected breakages:

```css title="global.css"
/* [!code --] */
@import 'chrona-ui/css/preset.css';
/* [!code ++] */
@import 'chrona-ui/css/preset-legacy.css';
```
...
This decision enables Chrona UI to iterate these elements without introducing unexpected breaking changes to usage outside of API surfaces.

**Migration Steps:** Override the relevant layout components with your own implementations, or utilize the Chrona CLI commands (`add` or `customize`) to install them locally.

### Removal of Internal Contexts

The following context modules are made private:

- `chrona-ui/contexts/sidebar`
- `chrona-ui/contexts/layout`

If you have already installed `<DocsLayout />` via Chrona CLI, import the legacy contexts instead:

```ts
// [!code --:2]
import {} from 'chrona-ui/contexts/sidebar';
import {} from 'chrona-ui/contexts/layout';

// [!code ++:2]
import {} from 'chrona-ui/legacy/sidebar';
import {} from 'chrona-ui/legacy/layout';
```
...
import { SidebarProvider } from 'chrona-ui/legacy/sidebar';
...
import layout components (e.g. `<DocsLayout />`) from `chrona-ui`, migrate your usages as follows:

```tsx
// [!code --]
import { useSidebar } from 'chrona-ui/contexts/sidebar';
// [!code ++]
...
```ts
// For docs layout
import { DocsPage } from 'chrona-ui/layouts/docs/page';
// For notebook layout
import { DocsPage } from 'chrona-ui/layouts/notebook/page';
```

**Migration Steps:** Update your imports accordingly. Although the default `chrona-ui/page` will redirect to the correct layout, we strongly advise explicit imports for clarity and future-proofing.
...
Chrona 16.2 aims to offer greater flexibility for developers. Welcome to share your thoughts about the new design on GitHub Discussion!
