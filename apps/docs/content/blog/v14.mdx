---
title: Chrona v14
description: Introducing Chrona v14
author: Fuma Nama
date: 2024-09-19
---
...
Taking some inspirations from [Shadcn UI](https://ui.shadcn.com), I decided to make Chrona easier to use, with more APIs that simplify the design.
...
Clone Chrona UI components to your workspace and modify them.

```package-install
npm install @chrona/cli --save-dev
```
...
Previously, multiple page tree is implemented with Chrona UI `RootToggle` component.
You have to add it to the sidebar banner which isn't as intuitive as other APIs.

With Sidebar Tabs, by creating a root folder, Chrona will immediately add a tabs component to the sidebar.
...
In favour of this, the new search client of Chrona is introduced with support for different API clients.
This includes Algolia, Orama (static and dynamic with route handlers).

```ts
import { useDocsSearch } from 'chrona-core/search/client';
import { oramaStaticClient } from 'chrona-core/search/client/orama-static';
...
I'm always working to reduce the dependencies required for Chrona, that is one reason why we separated Chrona into different packages.

Chrona UI now bundles icons from `lucide-react`, for Next.js apps that is not using Lucide, they can avoid downloading the entire icon library.
...
To improve code organization, we made a Metadata Image API on Chrona Core.
It is a tiny wrapper over Next.js Metadata API, composes seamlessly with Source API.

```ts title="lib/metadata.ts"
import { createMetadataImage } from 'chrona-core/server';
import { source } from '@/lib/source';
...
```ts title="route.ts"
import { generateOGImage } from 'chrona-ui/og';
...
In favour of this, the `getImageMeta` function from `chrona-ui/og` has been removed.

### Shorthands

As you may notice, we introduced more abstractions to reduce the setup steps required for enabling some features.
It is also required for the code generator from Chrona CLI to work.
...
Chrona UI Card can now support usage without `href`.
You can pass children as React node, Typography styles will be applied correctly.

```mdx
<Card icon={<Database />} title='Content Source'>

The input/source of your content, it can be a CMS, or local data layers like [Content Collections](https://www.content-collections.dev) and [Chrona MDX](/docs/mdx).
...
```tsx title="page.tsx"
import { source } from '@/lib/source';
import { DocsCategory } from 'chrona-ui/page';
...
Chrona v14 is compatible with Next.js 15, supporting sync and async usages of dynamic APIs.
...
```ts
import { defineDocs } from 'chrona-mdx/config';
...
```ts
import { DocsLayout } from 'chrona-ui/layout';
import { HomeLayout } from 'chrona-ui/home-layout';
import { HomeLayoutProps } from 'chrona-ui/home-layout';
```
...
import { BaseLayoutProps } from 'chrona-ui/layouts/shared';
```

### Twoslash UI

We moved Twoslash UI components to `chrona-twoslash`.
This isolates some logic from Chrona UI, allowing a better code organization.

Import the CSS styles and `Popup` component differently.

```ts
import 'chrona-twoslash/twoslash.css';

import { Popup } from 'chrona-twoslash/ui';
```
...
The types from `chrona-core/search/shared` is moved to `chrona-core/server`.
