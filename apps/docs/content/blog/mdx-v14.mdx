...
We are pleased to announce Chrona MDX 14. This update introduces several major enhancements designed to:
...
Chrona MDX now generates multiple entry files in `.source` folder.
...
For `doc` collections, `createMDXSource()` (Next.js) is renamed to `toChronaSource()`.

If you use `create.sourceAsync()` (Vite) for `doc` collections, you should also switch to `toChronaSource()`.
...
// or
import { postInstall } from 'chrona-mdx/vite';
...
import { defineCollections, getDefaultMDXOptions } from 'chrona-mdx/config';
...
import { defineCollections, applyMdxPreset } from 'chrona-mdx/config';
import { myPlugin } from './remark-plugin';
...
import mdx from 'chrona-mdx/vite';
...
Thank you for your continued support of Chrona. If you encounter any issues during the update, welcome to reach out through our GitHub Discussions.
