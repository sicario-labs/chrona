...
```ts
import createMDX from 'chrona-mdx/config';
...
Now you can generate types using `chrona-mdx` command, it's recommended to set it as a `postinstall` script.

```json title="package.json"
{
  "scripts": {
    "postinstall": "chrona-mdx"
  }
}
```
...
Now to integrate it with Chrona framework, change your `source.ts` from:

```ts
import { map } from '@/.map';
import { createMDXSource } from 'chrona-mdx/runtime/next';
...
```ts
import { defineDocs, defineCollections, frontmatterSchema } from 'chrona-mdx/config';
import { z } from 'zod';
...
You can read the latest docs of Chrona MDX for details.

<Cards>
  <Card href="/docs/mdx" title="Chrona MDX">
