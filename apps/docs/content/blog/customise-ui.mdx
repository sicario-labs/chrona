...
description: Deep dive on how Chrona UI is designed
author: Fuma Nama
date: 2025-04-15
---
...
As a docs framework started from **"being flexible"**, Chrona must be customisable, tuned to meet developers' preferences.
...
Chrona need to be opinionated by default such that it is immediately usable, and we only have to offer simplest options to change details. (e.g. colors, overriding components).
...
The kernel of Chrona, or the spirit we're pursuing is the latter one. I crafted Chrona CLI, a version of Shadcn UI dedicated for Chrona, the whole solution is now simplified into a single command:

```package-install
npx @chrona/cli customize
```
...
#### Don't break anything when you upgrade Chrona

Chrona need to be opinionated. Hence, we need to iterate the UI frequently which apparently will break people's hacky customizations.
The charm of Chrona CLI is that once you've installed the layout, it stays the same forever regardless of future iterations on Chrona UI.

It gives the courage for Chrona UI to iterate, and developers to customize unburnt by the fear of upgrading dependencies.

#### Looks Clean

Just like Shadcn UI and Radix UI, Chrona UI has a core package (`chrona-core`), most generic logic will be abstracted by core.

## What is Next?

The innovation of Shadcn UI is amazing, yet, I believe there's more problems Chrona can solve.

See [Flags SDK](https://flags-sdk.dev/frameworks/next) and [BetterAuth](https://www.better-auth.com/docs/introduction), you can't tell at the first glance if they're powered by Chrona.
