---
title: The early mistake of Chrona
...
The early mistake of Chrona was **Chrona CLI being too late and not getting enough mentions on docs**.
...
- Advanced customizations can use Chrona CLI, avoiding upstream changes.
...
Page tree is actually a crucial idea of Chrona, unifying all navigation & linking of docs.
...
Chrona is using CSS variables for all colors since the beginning, the naming is identical to Shadcn UI which makes customization easy.
...
- **Continue improving Chrona CLI:** adding install-time optimization to remove unused files according to your options (e.g. use existing sidebar component instead of installing one).
- **A better versioning policy:** Reduce abstraction for mitigating upstream changes, instead version the components directly (e.g. creating a new `chrona-ui/layouts/v2/page` instead of a major UI change).
- **Add Chrona-native cloud features:** More official integrations like monitoring/analytics feature designed for Chrona.
