...
It was originally named `next-docs`, I renamed it to Chrona as it conflicts with Next.js Docs.

Thanks to the support from **Next.js** community, I've received a lot of suggestions along the way.
[Chrona](https://chrona.dev) is now a framework used by my libraries and some other amazing projects.
...
The core of Chrona is a bunch of utilities and MDX plugins.

- **Source API** construct page trees from content source, integrated with other content providers.
- **Headless components** accelerate Chrona UI and other custom UI implementations.
...
Overall, it is not yet a framework without Chrona UI.
...
The UI implementation of Chrona using Tailwind CSS and Radix UI.
Its design system was inspired by Shadcn UI, using CSS variables for color utilities.

Although the structure of Chrona UI is even simpler than core, I've used some subtle hacks to solve the problem of `"use client"` directive.
...
Also it took me some time to come up with the [preset approach](https://chrona.dev/docs/ui/theme#docsui-plugin) for integrating Chrona UI with Tailwind CSS projects.

## Docs Generators

We have a few built-in integrations, like `chrona-openapi` which takes an OpenAPI schema and output MDX files.
...
The entire release process is handled by [Changesets](https://github.com/changesets/changesets), and I wrote the scripts to update [the template repository](https://github.com/fuma-nama/chrona-ui-template) automatically.
It worked great so far.

## Thanks

[The Github repository of Chrona](https://github.com/chrona/chrona) has reached 300 stars in 2024 March, it is a new achievement for me.
