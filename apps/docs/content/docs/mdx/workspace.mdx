---
title: Workspace
description: Use Chrona MDX in multiple workspace.
---

## Overview

**Workspace** in Chrona MDX, refers to an independent project with its own config and content.
...
<Callout title="Good to Know">
  Chrona MDX Workspace is not limited to the traditional meaning of workspace
  in package managers.
...
To integrate [multi-source](/docs/headless/source-api/source#multiple-sources) in Chrona:
...
export const source = loader(
  {
    root: docs.toChronaSource(),
    'my-workspace': MyWorkspace.docs.toChronaSource(),
  },
  {
    baseUrl: '/docs',
  },
...
In some setups, you might have multiple Chrona MDX configs with their own content directory.

With workspaces, you can integrate them into one Chrona MDX config, and access all collections of each workspace.
...
- when a deployment is triggered on $A$:
  - Chrona MDX handles each $T_n$ as a workspace.
  - each workspace generates its own collection entries, e.g. `collections/{repo}/server`.
  - Chrona `loader()` integrate multiple sources into one.
