...
You can run `chrona-mdx` to generate types & entry files (the `.source` folder).
...
```npm
npx chrona-mdx "config-path" "output-dir"
```

- `config-path`: Chrona MDX config file, default to `source.config.ts`.
