...
```tsx
import { DocsLayout, type DocsLayoutProps } from 'chrona-ui/layouts/docs';
import type { ReactNode } from 'react';
import { baseOptions } from '@/lib/layout.shared';
import { source } from '@/lib/source';
...
'use client';
import * as Base from 'chrona-ui/components/sidebar/base';
...
Chrona use the `<Link />` component of your React framework, and keep their default prefetch behaviours.
...
Handling layout is challenging, Chrona UI needed an approach that is:
...
Chrona UI does this with a grid system:
