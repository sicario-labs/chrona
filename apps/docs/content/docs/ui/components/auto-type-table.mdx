...
<auto-type-table id="example" name="AutoTypeTableExample" type='export interface AutoTypeTableExample {
    ...
    * See https://chrona.dev/docs/ui/components/type-table
    **/
    name: boolean | null;
...
import { createGenerator, createFileSystemGeneratorCache } from 'chrona-typescript';
import { AutoTypeTable, type AutoTypeTableProps } from 'chrona-typescript/ui';

const generator = createGenerator({
  // set a cache, necessary for serverless platform like Vercel
  cache: createFileSystemGeneratorCache('.next/chrona-typescript'),
});
