...
Chrona provides many useful extensions to MDX, a markup language. Here is a brief introduction to the default MDX syntax of Chrona.

> MDX is not the only supported format of Chrona. In fact, you can use any renderer such as headless CMS.
>
...
Chrona support YAML-based frontmatter by default, `title` represents the page title (`h1`) in Chrona UI.
...
```ts tab="Chrona MDX" title="source.config.ts"
...
import { remarkCodeTab } from 'chrona-core/mdx-plugins';
...
> This is only available on **Chrona MDX**.
