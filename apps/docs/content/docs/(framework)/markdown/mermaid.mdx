...
Chrona doesn't have a built-in Mermaid wrapper provided, we recommend using `mermaid` directly.

You can decide the Mermaid renderer to configure:

### Official Renderer

Install the required dependencies, `next-themes` is used with Chrona to manage the light/dark mode.
...
import { CodeBlock, Pre } from 'chrona-ui/components/codeblock';
...
```tsx tab="Chrona MDX" title="source.config.ts"
import { remarkMdxMermaid } from 'chrona-core/mdx-plugins';
