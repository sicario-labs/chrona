...
You can integrate a simple feedback system with Chrona.

## Installation

Install it using **Chrona CLI**.

```npm
npx @chrona/cli@latest add feedback
```
...
```tsx
import { DocsPage } from 'chrona-ui/layout/docs/page';
...
```tsx title="source.config.ts (Chrona MDX)"
import {
  remarkBlockId,
  type RemarkBlockIdOptions,
} from 'chrona-core/mdx-plugins/remark-block-id';
