...
Chrona offers the **MDX Remote** package, it is a helper to integrate Markdown/MDX content sources with Chrona.
You can think it as a `next-mdx-remote` with built-in plugins for Chrona.
...
```tsx
import { createCompiler } from '@chrona/mdx-remote';
...
```tsx
import { createCompiler } from '@chrona/mdx-remote';
import { executeMdxSync } from '@chrona/mdx-remote/client';
import { DocsBody, DocsPage } from 'chrona-ui/layouts/docs/page';
import { getPage } from './my-content-source';
