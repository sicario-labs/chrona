---
title: Obsidian
description: Render an Obsidian vault directly with Chrona.
---

`chrona-obsidian` exposes a runtime content source. It reads and compiles the vault directly, so it does not generate intermediate MDX files or need a Chrona MDX remark plugin.
...
```package-install
chrona-obsidian shiki
```

### Generate Styles

The components from `chrona-obsidian/ui` are styled with Tailwind CSS. Add the following line so their classes are generated:
...
/* [!code ++] */
@import 'chrona-obsidian/css/preset.css';
```
...
import { obsidian } from 'chrona-obsidian';
...
import defaultMdxComponents, { createRelativeLink } from 'chrona-ui/mdx';
import * as ObsidianComponents from 'chrona-obsidian/ui';
...
{
  "scripts": {
    "dev": "chrona-obsidian dev -- next dev"
  }
}
```
...
import { localContentPlugin } from 'chrona-obsidian/dev/vite';
...
```ts title="lib/source.ts"
import { watchWithVite } from 'chrona-obsidian/dev/vite';
...
```ts title="lib/source.ts"
import { frontmatterSchema, obsidian } from 'chrona-obsidian';
import { metaSchema } from 'chrona-core/source/schema';
import { z } from 'zod';
