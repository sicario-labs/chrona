...
/* [!code ++] */
@import 'chrona-openapi/css/preset.css';
```
...
import { openapiPlugin } from 'chrona-openapi/server';
...
import { generateFiles } from 'chrona-openapi';
...
export const Route = createFileRoute('/docs/$')({
  ...
  loader: async ({ params }) => {
    ...
    // Chrona MDX: only preload content for normal pages [!code highlight:3]
    if (data.type === 'docs') {
...
function Page() {
  const page = useChronaLoader(Route.useLoaderData());
...
You can see the full [Tanstack Start example](https://github.com/chrona/chrona/blob/dev/examples/tanstack-start-openapi).
