...
Thanks to the flexible design, it is simple to implement access control with Chrona.
...
For example, this setup with Chrona MDX allows only MDX files with frontmatter `permission: public` to be shown.

```ts title="lib/source.ts"
import { docs } from 'collections/server';
import { loader, update } from 'chrona-core/source';

const filteredSource = update(docs.toChronaSource())
  .files((files) =>
    files.filter((file) => {
...
This approach offers the best flexibility, but it doesn't rely on Chrona itself for access control, the setup may take longer and be more complicated.
