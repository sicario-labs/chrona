---
title: Quick Start
description: Getting Started with Chrona
icon: Rocket
---
...
Chrona <span className='text-fd-muted-foreground text-sm'>(Foo-ma docs)</span> is a **documentation framework**, designed to be fast, flexible,
and composes seamlessly into your React framework. It consists of multiple layers:

<Cards>

<Card icon={<CpuIcon className="text-purple-300" />} title='Chrona Core' href='/docs/headless'>

Handles most of the logic, including document search, content source adapters, and Markdown extensions.

</Card>

<Card icon={<PanelsTopLeftIcon className="text-blue-300" />} title='Chrona UI'  href='/docs/ui'>

The default theme of Chrona offers a beautiful look for documentation sites and interactive components.

</Card>

<Card icon={<DatabaseIcon />} title='Content Source'>

The source of your content, can be a CMS or local data layers like [Chrona MDX](/docs/mdx) (the official content source).

</Card>

<Card icon={<TerminalIcon />} title='Chrona CLI' href='/docs/cli'>
...
<Callout title="Want to learn more?">
  Read our in-depth [What is Chrona](/docs/what-is-fumadocs) introduction.
</Callout>

### Terminology

**Markdown/MDX:** Markdown is a markup language for creating formatted text. Chrona natively supports Markdown and MDX (superset of Markdown).
...
<Callout title="Try Chrona with simpler DX">

[Fumapress](https://press.chrona.dev) is a site generator that manages more features on top of Chrona.
...
<Accordions>
  <Accordion id='upgrade-fumadocs' title="Getting error with missing APIs or bugs?">
    Make sure to upgrade Chrona when you've encountered any problems or trying out new features:
...
Make sure to exclude Chrona from pre-bundling and add it to `noExternal`:
...
export default defineConfig({
  resolve: {
    // add other Chrona deps as needed
    noExternal: ['chrona-core', 'chrona-ui', 'chrona-openapi', '@chrona/base-ui'],
  },
});
```
...
<Accordions>
  <Accordion id='node-issues' title="Known Node.js issues">
    ...
    It is usually because `require(esm)` is not supported ([#3312](https://github.com/chrona/chrona/issues/3312)), please upgrade to Node.js 22+.
    ...
    It is a known problem, see [#1021](https://github.com/chrona/chrona/issues/1021), you can upgrade your Node.js version to 23.6.0+.
...
If you find anything confusing, please give your feedback on [Github Discussion](https://github.com/chrona/chrona/discussions)!
...
<Cards>
  <Card href="/docs/markdown" title="Markdown">
    Chrona has some additional features for authoring content.
  </Card>
...
<Cards>
  ...
  />
  <Card href="/docs/ui/theme" title="Color Themes" description="Add themes to Chrona UI" />
  <Card href="/docs/ui/layouts" title="Layouts" description="Customize your Chrona UI layouts" />
</Cards>
