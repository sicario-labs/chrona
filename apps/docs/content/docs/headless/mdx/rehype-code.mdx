...
A wrapper of [`@shikijs/rehype`](https://shiki.style/packages/rehype), the syntax highlighter used by Chrona.
...
import { rehypeCode, type RehypeCodeOptions } from 'chrona-core/mdx-plugins';
...
export default defineConfig({
  mdxOptions: {
    rehypeCodeOptions: {
      // enabled by default on Chrona MDX
...
```ts
import { type RehypeCodeOptions } from 'chrona-core/mdx-plugins';
