...
description: Use Content Collections for Chrona
---
...
To integrate with Chrona, add the following to your `content-collections.ts`.
...
To use components from other packages like Chrona UI, pass them to your `<MDXContent />` component.
...
<Callout title='Deep Dive: Why?'>
    ...
    To support importing a package from node modules, Chrona added a default value to the `cwd` option of MDX Bundler.
