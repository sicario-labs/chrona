...
description: Built-in document search of Chrona
---

Chrona supports document search with [ZBSearch](https://www.zbsearch.dev), It is the default but also the recommended option since it can be self-hosted and totally free.
...
Usually, it is provided by your content source (e.g. Chrona MDX). You can also extract it from Markdown/MDX document using the [Remark Structure](/docs/headless/mdx/structure) plugin.

```ts tab="Next.js" title="app/api/search/route.ts"
import { source } from '@/lib/source';
import { createSearchAPI } from 'chrona-core/search/server';
...
- **Chrona UI**: Supported out-of-the-box, see [Search UI](/docs/search/orama) for details.
- **Search Client**:
...
<auto-type-table type='import("chrona-core/search/client/fetch").FetchOptions' />
...
- **Chrona UI**: Configure [Tag Filter](/docs/search/orama#tag-filter) on Search UI.
...
- **Chrona UI**: use [static client](/docs/search/orama#static) on Search UI.
...
- **Search Client**: use `staticClient` instead of `fetchClient`.
  ...
  <auto-type-table type='import("chrona-core/search/client/orama-static").StaticOptions' />
...
import { source } from '@/lib/source';
import { createI18nSearchAPI } from 'chrona-core/search/server';
...
- **Chrona UI**: No changes needed, Chrona UI handles this when you have i18n configured correctly.
- **Search Client**:
...
```ts
import { initAdvancedSearch } from 'chrona-core/search/server';
