...
description: Integrate Typesense Search with Chrona
---
...
```package-install
typesense typesense-chrona-adapter
```
...
import { findPath } from 'chrona-core/page-tree';
import type { DocumentRecord } from 'typesense-chrona-adapter';
...
import { sync, DocumentRecord } from 'typesense-chrona-adapter';
...
- Use [Chrona UI search dialog](/docs/search/typesense).
- Build your own search UI with the Typesense search client hook.

  ```ts
  import { Client } from 'typesense';
  import { useTypesenseSearch } from 'typesense-chrona-adapter/client';
...
- **Chrona UI**: Enable [Tag Filter](/docs/search/typesense#tag-filter) on Search UI.
