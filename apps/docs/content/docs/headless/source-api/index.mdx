...
`loader()` provides an interface for Chrona to integrate with different content sources.
...
You can use it with content sources like Chrona MDX, see [`source`](/docs/headless/source-api/source) for details.
...
function MyComponent() {
  ...
  const { pageTree, ...data } = useChronaLoader(useLoaderData());

  // now render it (e.g. via Chrona UI)
