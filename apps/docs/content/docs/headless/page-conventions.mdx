...
> This guide only applies for content sources that uses `loader()` API, such as **Chrona MDX**.

## Overview

Chrona generates **page slugs** and **page tree** (sidebar items) from your content directory using [`loader()`](/docs/headless/source-api),
...
Chrona detects from the following properties to construct page trees.
...
Page information is supplied by the content source such as **Chrona MDX**.

On Chrona MDX, you can specify a [`schema`](/docs/mdx/collections#schema-1) option to customize frontmatter schema.
...
<Callout title='Chrona UI'>

Chrona UI renders root folders as [Layout Tabs](/docs/ui/layouts/docs#layout-tabs), which allows users to switch between them.
...
This is intentional, such that Chrona can locate the active tree item purely based on current pathname.
...
Since Chrona doesn't include an icon library, you have to convert the icon names to JSX elements in runtime, and render it as a component.
