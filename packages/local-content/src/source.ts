...
import type { DynamicLoader } from 'chrona-core/source/dynamic';
...
export interface LocalSource<Page extends PageData, Meta extends MetaData> extends WatchableSource {
  ...
  invalidateAll: () => void;
  /**
   * @deprecated import `watchWithDevServer` from `@chrona/local-content/dev/ws`,
   * or `watchWithVite` from `@chrona/local-content/dev/vite`, instead.
   */
