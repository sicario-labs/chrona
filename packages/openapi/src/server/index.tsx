...
export interface OpenAPIServer {
  ...
  /**
   * Generate virtual pages for Chrona Source API (note: please disable cache to allow built-in revalidation)
   */
  dynamicSource: (
    options?: OpenAPISourceOptions,
  ...
  /**
   * Chrona Source API integration, pass this to `plugins` array in `loader()`.
   */
...
export function createOpenAPI(options: OpenAPIOptions = {}): OpenAPIServer {
  ...
  function getSchema(schemaId: string): Promise<LoadedDocument> {
    if (!(schemaId in resolvedInput)) {
      console.warn(
        `[Chrona OpenAPI] the document "${schemaId}" is not listed in the input array, this may be unexpected and won't be cached properly.`,
      );
