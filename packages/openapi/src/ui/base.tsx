...
import { remarkGfm } from 'chrona-core/mdx-plugins/remark-gfm';
...
import type { ShikiFactory } from 'chrona-core/highlight/shiki';
import type { GeneratedPageProps } from '@/utils/pages/builder';
import { Markdown } from './components/markdown';
import { Schema, type SchemaUIOptions } from '@chrona/api-docs/components/schema';
...
export function createOpenAPIPageBase({
  ...
  return function OpenAPIPage(props) {
    ...
    if ('preloaded' in props) {
      ...
      if (!doc)
        throw new Error(
          `[Chrona OpenAPI] the document ${props.document} is not preloaded, make sure to pass the "preloaded" prop to <OpenAPIPage />`,
        );
  ...
  let content = renderPageLayout(
    {
      operations: operations?.map((item) => {
        ...
        if (!pathItem)
          throw new Error(`[Chrona OpenAPI] Path not found in OpenAPI schema: ${item.path}`);

        const operation = pathItem[item.method];
        if (!operation)
          throw new Error(
            `[Chrona OpenAPI] Method ${item.method} not found in operation: ${item.path}`,
          );
      ...
      webhooks: webhooks?.map((item) => {
        ...
        if (!webhook)
          throw new Error(`[Chrona OpenAPI] Webhook not found in OpenAPI schema: ${item.name}`);

        const hook = webhook[item.method];
        if (!hook)
          throw new Error(
            `[Chrona OpenAPI] Method ${item.method} not found in webhook: ${item.name}`,
          );
