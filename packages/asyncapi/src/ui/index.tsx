...
import { defaultShikiFactory } from 'chrona-core/highlight/shiki/full';
...
export function createAsyncAPIPage({
  ...
  return function AsyncAPIPage(props) {
    ...
    if ('preloaded' in props) {
      ...
      if (!doc)
        throw new Error(
          `[Chrona AsyncAPI] the document ${props.document} is not preloaded, make sure to pass the "preloaded" prop to <AsyncAPIPage />`,
        );
    } else {
    ...
    const ctx: RenderContext = useMemo(() => {
      ...
      return {
        ...
        storageKeyPrefix: options.storageKeyPrefix ?? 'chrona-asyncapi-',
      };
    }, [processed]);
