...
/**
 * Process input to a Chrona GraphQL compatible format, input can be:
 *
...
export async function loadSchema(input: GraphQLSchemaInput): Promise<LoadedSchema> {
  try {
    ...
    throw new Error(
      `[Chrona GraphQL] Failed to load schema: ${Array.isArray(input) || typeof input === 'string' ? input : 'introspection result'}`,
      {
        cause: e,
      },
    );
