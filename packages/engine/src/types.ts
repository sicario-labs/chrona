...
export function getSymbolKind(node: Node): SymbolKind {
  if (node.getKindName().startsWith('Class')) return 'class';
  if (node.getKindName().startsWith('Interface')) return 'interface';
  if (node.getKindName().startsWith('TypeAlias')) return 'type-alias';
  if (node.getKindName().startsWith('Enum')) return 'enum';
  if (node.getKindName().startsWith('Function')) return 'function';
  if (node.getKindName().startsWith('Module')) return 'module';
  if (
    node.getKindName().startsWith('Variable') ||
    node.getKindName().startsWith('Binding')
  )
    return 'variable';
  return 'unknown';
}