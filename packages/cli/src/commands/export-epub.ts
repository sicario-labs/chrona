...
export async function exportEpub(options: {
  ...
  if (!hasNext && framework === 'next') {
    console.error(
      picocolors.red(
        'Next.js project not found. Run this command from a Chrona Next.js project root.',
      ),
    );
    process.exit(1);
  }
  ...
  if (options.scaffoldOnly) {
    ...
    console.log('  1. Add chrona-epub to your dependencies: pnpm add chrona-epub');
    ...
    if (framework === 'next') {
      ...
      console.log('  6. Run: chrona export epub --framework next');
    } else {
      console.log(`  3. Add a prerender route that outputs EPUB to ${buildPath}`);
      console.log('  4. Run production build: pnpm build');
      console.log(`  5. Run: chrona export epub --framework ${framework}`);
    }
    return;
  }

  // Check for chrona-epub dependency
  if (!pkg) {
    console.error(
    ...
    try {
      ...
      const stderr =
        err && typeof err === 'object' && 'stderr' in err
          ...
          : '';
      console.error(picocolors.red(`Failed to install chrona-epub. Command: ${installCmd}`));
