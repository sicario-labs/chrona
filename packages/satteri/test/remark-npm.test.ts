...
describe('remark-npm', () => {
  it('expands package-install blocks into package manager tabs', async () => {
    ...
    expect(code).toContain('npm install chrona-core');
    expect(code).toContain('pnpm add chrona-core');
    expect(code).toContain('yarn add chrona-core');
    expect(code).toContain('bun add chrona-core');
  });
