...
import type { MetaData, PageData } from 'chrona-core/source';
import type { metaSchema, pageSchema } from 'chrona-core/source/schema';
...
function macroError(): Error {
  return new Error(
    '[MDX] this macro was not compiled by the bundler plugin of `chrona-mdx`. To use `chrona-mdx/macro`, set the `include` option on your bundler plugin, and make sure this module matches its patterns.',
  );
}
