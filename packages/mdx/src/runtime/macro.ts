import type { MetaData, PageData, Source } from 'chrona-core/source';
...
import { server, toChronaSource } from './server';
...
export interface MacroDocCollection<Frontmatter = unknown, Extra = unknown> {
  ...
  toChronaSource: (options?: ToChronaSourceOptions) => Source<{
...
interface ToChronaSourceOptions {
...
export async function doc(args: BaseArgs & { entries: GlobEntries }): Promise<MacroDocCollection> {
  ...
  return {
    ...accessor(entries),
    toChronaSource(options) {
      return toChronaSource(entries, [], options);
    },
  };
}
...
export async function docs(
  ...
  return {
    ...
    getMeta: getMeta.get,
    toChronaSource(options) {
      return toChronaSource(docEntries, metaEntries, options);
    },
  };
}
