...
import type { SatteriPresetOptions } from '@chrona/satteri/preset';
import type { ExtraPluginHooks } from '@chrona/satteri/compile';
...
export async function buildSatteriMDX(
  ...
  const [{ compileMdx }, { remarkLlms }, { remarkInclude }] = await Promise.all([
    import(/* turbopackOptional: true */ '@chrona/satteri/compile'),
    import(/* turbopackOptional: true */ '@chrona/satteri/remark-llms'),
    import(/* turbopackOptional: true */ '@chrona/satteri/remark-include'),
  ]);
...
async function resolvePresetOptions(
  ...
  const { applySatteriPreset } = await import(
    /* turbopackOptional: true */ '@chrona/satteri/preset'
  );
