...
export interface PluginOptions extends Pick<CoreOptions, 'configPath' | 'outDir' | 'plugins'> {
  /**
   * Configure the macro API (`chrona-mdx/macro`), or `false` to disable it.
   *
  ...
  /**
   * Update Vite config to fix module resolution of Chrona
   *
   * @defaultValue true
   */
...
export interface ChronaMdxOptions extends PluginOptions {
...
export function chronaMdx(options?: ChronaMdxOptions) {
  ...
  return [
    {
      ...
      async config(config, env) {
        ...
        const { getConfig } = await import('@chrona/vite');
